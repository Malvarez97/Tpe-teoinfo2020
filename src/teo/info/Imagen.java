...
public class Imagen {
    ...
    public int[][] getValoresMat(String nombrefoto) {// ¿podriamos agregarle que ponga el nombre de la foto a ingresar y lo haga automatico del Main ?
        // creo la imagen
        BufferedImage imagen = this.getImage("/"+nombrefoto+".jpg");//lee lo que hay en la carpeta imagenes ..curiosidad la lee de la carpeta la primera vez y hacer una copia de "segurirdad en la carpeta out de produccion, si la borras , queda guardad en esa copia y se puede volver a usar .
        ...
        for (int alto = 0; alto < imagen.getHeight(); alto++) {
            for (int ancho = 0; ancho < imagen.getWidth(); ancho++) {
                matriz[ancho][alto] = imagen.getRGB(ancho, alto);
            }
        }
